...
void main() {
  ...
  testWidgets('Document Classifier Integration Test using mock data from the backend_service_mock',
    ...
    await tester.pumpAndSettle();
    expect(find.byType(ListTile), findsNWidgets(2), reason: "The additional category should be deleted");
    ...
    expect(listWidgets.length, 3, reason: "The query should list all 3 documents");
    ...
    for (var element in listWidgets) {
      ...
      expect(circleAvatarFinder, findsOneWidget, reason: "Each document should have an avatar");
      ...
      expect(certainty, certaintyResults[i], reason: "The certainty should be calculated correctly");
      i++;
    }
    // Reset back to initial view
    await tester.enterText(find.byType(TextField), "");
    await tester.pump(const Duration(milliseconds: 600));
    expect(find.byType(ListTile), findsNWidgets(2), reason: "After resetting the search we should see one category and one document");
    ...
    await tester.pumpAndSettle();
    expect(find.byType(ListTile), findsNWidgets(1), reason: "After the document is deleted we should only have the category left");


    // Test if Error Popup opens
    await tester.enterText(find.byType(TextField), "ERROR");
    await tester.pump(const Duration(milliseconds: 600));
    expect(find.byType(AlertDialog), findsOneWidget, reason: "After triggering the alert the dialog should have popped up");
  });
}
